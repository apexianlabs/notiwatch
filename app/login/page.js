  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = tab === 'login'
        ? { email: form.email, password: form.password, product: 'notiwatch' }
        : { email: form.email, password: form.password, full_name: form.name, source_product: 'notiwatch' }
      ...
      document.cookie = `not_token=${token}; path=/; max-age=2592000; SameSite=Lax`
      document.cookie = `not_user=${encodeURIComponent(JSON.stringify({ id: user.id, email: user.email, name: fullName }))}; path=/; max-age=2592000; SameSite=Lax`
      ...
      if (tab === 'signup') {
        fetch(`${process.env.NEXT_PUBLIC_DB_API_URL}/email/welcome`, {
          ...
          body: JSON.stringify({ product: 'notiwatch', to: form.email, name: fullName, email: form.email })
      ...
      <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',padding:'32px 24px',minHeight:'100vh'}}>
        <div style={{width:'100%',maxWidth:400}}>
          ...
          <p style={{fontSize:14,color:'#64748b',marginBottom:20}}>
            {tab === 'login' ? `Sign in to your NotiWatch account.` : 'Start your free trial today.'}
          </p>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{width:'100%',padding:13,borderRadius:10,border:'none',background:'#ea580c',color:'#fff',fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'Inter,sans-serif',marginBottom:16,opacity:loading?0.7:1}}>
