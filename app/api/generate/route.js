...
const DB_API_KEY = process.env.DB_API_KEY_NOTIWATCH
...
export async function POST(request) {
  try {
    ...
    // Check usage limits
    if (userId) {
      const usageRes = await fetch(`${process.env.DB_API_URL}/usage/check?user_id=${userId}&product=notiwatch`, {
        headers: { 'Authorization': `Bearer ${process.env.DB_API_KEY_NOTIWATCH}` }
      })
    ...
    const aiRes = await fetch(`${AI_API_URL}/api/process`, {
      ...
      body: JSON.stringify({
        ...
        inputs: {
          ...
          monitor_type: monitor_type || 'general'
        }
      })
    })
    ...
    if (userId) {
      fetch(`${process.env.DB_API_URL}/usage/track`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.DB_API_KEY_NOTIWATCH}` },
        body: JSON.stringify({ user_id: userId, product: 'notiwatch', action: 'generate' })
      }).catch(() => {})
    }
    ...
    if (userId && DB_API_URL) {
      try {
        const dbRes = await fetch(`${DB_API_URL}/db/notiwatch/monitors`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${DB_API_KEY}` },
          body: JSON.stringify({
            user_id: userId,
            title: `${company_name} monitor`,
            url,
            company_name,
            result_data: result,
            status: 'complete'
